...
case "${MessageType:-"progress"}" in
	uiphase ) \
		printf $format $NOTBOLD $WHITEBG $BLACKFG "$ScriptName:($FunctionName) Displaying UI Message - $StatusMessage"  | $tee -a "${SCRIPT_LOG%%.log}.color.log" ;		
	...
	uistatus ) \
		...
		sleep ${CustomDelay:=1} ;;
	progress) \
		printf $format $NOTBOLD $WHITEBG $BLACKFG "$ScriptName:($FunctionName) - $StatusMessage"  | $tee -a "${SCRIPT_LOG%%.log}.color.log" ;		
		printf "%s\n" "$DATE $ScriptName:($FunctionName) - $StatusMessage" >> "${SCRIPT_LOG:?}" ;;
		# Used for general progress messages, always viewable

	notice) \
		printf $format $NOTBOLD $YELLOWBG $BLACKFG "$ScriptName:($FunctionName) - $StatusMessage" | $tee -a "${SCRIPT_LOG%%.log}.color.log" ;
		printf "%s\n" "$DATE $ScriptName:($FunctionName) - $StatusMessage" >> "${SCRIPT_LOG:?}" ;;
		# Notifications of non-fatal errors , always viewable

	error) \
		printf "%s\n\a" "$DATE $ScriptName:($FunctionName) - $StatusMessage" >> "${SCRIPT_LOG:?}" | $tee -a "${SCRIPT_LOG%%.log}.color.log";
	...
	header) \
		printf $format $NOTBOLD $BLUEBG $BLUEFG "$ScriptName:($FunctionName) - $StatusMessage"  | $tee -a "${SCRIPT_LOG%%.log}.color.log" ;
		printf "%s\n" "$DATE $ScriptName:($FunctionName) - $StatusMessage" >> "${SCRIPT_LOG:?}" ;;
		# Function and section headers for the script

	passed) \
		printf $format $NOTBOLD $GREENBG $BLACKFG "$ScriptName:($FunctionName) - $StatusMessage" | $tee -a "${SCRIPT_LOG%%.log}.color.log";
		printf "%s\n" "$DATE $ScriptName:($FunctionName) - $StatusMessage" >> "${SCRIPT_LOG:?}" ;;
...
settings(){
  declare KEY="$1"
  declare PLIST="$RunDirectory/settings"
  declare VALUE="$($defaults read "${PLIST%%.plist}" "$KEY" 2>/dev/null)"
  printf "%s\n" VALUE
}
...
begin(){
	...
	export InstallPhaseTxt="$(settings 'InstallProgressTxt')"
	export InstallProgressFile="$(settings 'InstallProgressTxt')"
	deleteBridgeFiles
}
